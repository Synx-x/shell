...
Singleton {
    ...
    function loadPinnedItems(): void {
        const pinned = GlobalConfig.launcher.pinnedClipboardItems || [];
        root.pinnedItems = pinned;
    }

    function savePinnedItems(): void {
        GlobalConfig.launcher.pinnedClipboardItems = root.pinnedItems;
    }
    ...
    function copyToClipboard(item): void {
        // Pass only the numeric id, as an argument. The content never reaches the shell.
        Quickshell.execDetached(["sh", "-c", "printf '%s' \"$1\" | cliphist decode | wl-copy", "sh", String(item.id)]);
